...
module.exports = {
  ...
  dontSeeToastMessage(type, string) {
    if (string === undefined) {
      I.waitForInvisible(locate(type).inside(locate(commonPage.messagePopup.toast), constants.SHORT_WAIT));
    } else {
      I.waitForInvisible(locate(type).withText(string).inside(locate(commonPage.messagePopup.toast)), constants.SHORT_WAIT);
    }
  },
};
