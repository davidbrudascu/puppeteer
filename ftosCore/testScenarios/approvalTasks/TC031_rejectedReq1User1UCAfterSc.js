...
Scenario('Reject a request with IsAdvisory=True, Competence Approval Mode=All with 1 User/1 User Competence which has an After Script.', async () => {
  ...
  approvalTaskActions.insertRecord(specificData.recordNameTC031);
  ...
  approvalTaskVerify.verifySingleApprovalTaskCreationAdvisoryTrue(
    specificData.recordNameTC031,
    specificData.approvalUsername1,
    commonData.competenceLevel1,
    commonData.taskStatusActive,
  );
  // Open Approval Task
  approvalTaskActions.openApprovalTask(specificData.approvalUsername1, specificData.recordNameTC031);
  // Approve the request
