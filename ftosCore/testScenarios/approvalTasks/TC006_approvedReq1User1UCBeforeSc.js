...
Scenario('Approve a request with IsAdvisory=True, Competence Approval Mode=All with 1 User/1 User Competence which has a Before Script.', async () => {
  ...
  approvalTaskActions.insertRecord(specificData.recordNameTC006);
  ...
  approvalTaskVerify.verifySingleApprovalTaskCreationAdvisoryTrue(
    specificData.recordNameTC006,
    specificData.approvalUsername1,
    commonData.competenceLevel1,
    commonData.taskStatusActive,
  );
  // Open Approval Task
  approvalTaskActions.openApprovalTask(specificData.approvalUsername1, specificData.recordNameTC006);
  // Approve the request
