...
Scenario('Approve a request with IsAdvisory=True, Competence Approval Mode=All with 1 User/1 User Competence.', async () => {
  ...
  approvalTaskActions.insertRecord(specificData.recordNameTC001);
  ...
  approvalTaskVerify.verifySingleApprovalTaskCreationAdvisoryTrue(
    specificData.recordNameTC001,
    specificData.approvalUsername1,
    commonData.competenceLevel1,
    commonData.taskStatusActive,
  );
  // Open Approval Task
  approvalTaskActions.openApprovalTask(specificData.approvalUsername1, specificData.recordNameTC001);
  // Approve the request
