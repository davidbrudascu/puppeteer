...
Scenario('Return (Need More Info) a request with IsAdvisory=True, Competence Approval Mode=All with 1 User/1 User Competence.', async () => {
  ...
  approvalTaskActions.insertRecord(specificData.recordNameTC036);
  ...
  approvalTaskVerify.verifySingleApprovalTaskCreationAdvisoryTrue(
    specificData.recordNameTC036,
    specificData.approvalUsername1,
    commonData.competenceLevel1,
    commonData.taskStatusActive,
  );
  // Open Approval Task
  approvalTaskActions.openApprovalTask(specificData.approvalUsername1, specificData.recordNameTC036);
  // Approve the request
