...
// Data
const data = require('~data/digitalAsset/TC050_InsertMenuItemFromCI.json');
// Verify
...
Scenario('User can add a Menu Item to CI tab', async () => {
  ...
  // Add a Menu Item extension from configuration items
  await digitalAssetActions.addDigitalAssetConfigurationItem(
    data.displayName,
    data.menuItem,
    data.configItemsType,
  );
  // Verify if Menu Item extension was added in configuration items
  await digitalAssetVerify.verifyConfigurationItems(data.recordName, data.menuItem);
